...
word double_click_delay_count, long_key_count, key_count,mickey_count;
...
bit flag_1click			:r_key_flag.7

byte r_mickey_flag;
bit flag_mickey         :r_mickey_flag.0
bit flag_mickey_press   :r_mickey_flag.1
bit flag_mickey_active  :r_mickey_flag.2

//--------------------------------------------------客制变量
byte click_time_count, click_count,mick_time_count,mick_count;
word key_5c_count;
word mick_5c_count;
...
void mickey_scan(void)
{
	if((!flag_mickey && p_mickey) || ((flag_mickey) && (!p_mickey)))
	{
		mickey_count ++;
		if(flag_mickey)
		{
			if(mickey_count >= (mickey_delay_time + 3))
			{
				mickey_count = 0;
				flag_mickey = 0;
			}
		...
		{
			if(mickey_count >= mickey_delay_time)
			{
				mickey_count = 0;
				flag_mickey = 1;
				flag_mickey_press = 1;//此处置咪头标志位，到外面用完就清掉
			}
		}

		flag_mickey_active = 1;
	}
	else
	{
		mickey_count = 0;
		flag_mickey_active = 0;
	}
	//吸的次数
	if(flag_mickey_press)
	{
		if(mick_time_count == 0)//五连击的第一击
		{
			mick_time_count ++;
			mick_count = 1;			//单击次数为1
			mick_5c_count = 0;			//连击总计时清零
		}
		else	//非第一击
		{
			mick_5c_count += mick_time_count;//2.5S计时
			if(mick_5c_count < 500)
			{
				mick_count ++;
				if(mick_count>4)	//单击计数+1,判断连击次数
				{
					flag_5click = 1;	//确认5连击
					mick_time_count = 0;
				}
	...
	else
	{
		...
		{
			mick_time_count ++;
			if(mick_time_count>100)	//点击间隔超过0.5S视作超时
			{
				mick_time_count = 0;
				if(!flag_mickey)
				{
					...
					else if((mick_5c_count < 270) && (mick_count == 3))
					{
						flag_3click = 1;
					}
				}
